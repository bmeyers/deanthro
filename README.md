...
This is an an attempt at implementing the paper, [De-anthropomorphizing “AI”: From wishful mnemonics to accurate nomenclature](https://doi.org/10.5210/fm.v31i2.14366), by Nanna Inie, Peter Zukerman, and Emily M. Bender, recognizing that these systems generate text that consistently violates the standards put forth in the paper. While the paper is focused on how humans discuss these systems, it is interesting to consider how the models themselves might be "encouraged" to produce de-anthropomorphized outputs. This should be considered a research project rather than a polished tool. Please let me know if you have feedback or suggestions.

I used `claude-4-5-sonnet` to generate and inital [markdown file](./deanthropomorphize.md) based on the paper, essentially a text transformation and summary task. I then edited the resulting markdown based on my understanding of the paper. 

4/23/26: I shortened the file by instructing Claude to reduce the lenghth of the file, followed by editing and changing the resulting file. I removed some additional sections that the LLM had left in and expanded the substitution table with specfic examples from the paper.
...
### Method 5: Claude Code Skill (Automatic)
...
**Clone the repo (project-local skill):**
Opening this repository in Claude Code automatically activates the `deanthro` skill via the `.claude/skills/` directory. No configuration needed.
